...
exports.getPreviousRuns = exports.getNextRuns = void 0;
...
function resolveTimezone(options) {
    ...
    if (options === null || options === void 0 ? void 0 : options.locale) {
        const locale = typeof options.locale === 'string' ? (0, index_1.getLocale)(options.locale) : options.locale;
...
function getNextRuns(expression, count, fromDate = new Date(), options) {
...
function getPreviousRuns(expression, count, fromDate = new Date(), options) {
    if (!Number.isInteger(count) || count <= 0) {
        throw new Error('count must be a positive integer');
    }
    if (!(fromDate instanceof Date) || Number.isNaN(fromDate.getTime())) {
...
exports.getPreviousRuns = getPreviousRuns;
/**
 ...
 */
function computeSearchLimit(precision, count) {
    const yearBudget = 4 * count + 2;
    const unitsPerDay = precision === 'minute' ? 1440 : 86400;
    return yearBudget * 366 * unitsPerDay;
}
function parseExpression(expression) {
    const expanded = helper_1.ExpressionHelper.expandMacro(expression);
    const parts = expanded.trim().split(/\s+/);
    if (parts.length === 5) {
        validator_1.CronValidatorU2Q.validateUnix(expanded);
        return { kind: 'unix', parts };
    }
    if (parts.length !== 6 && parts.length !== 7) {
        throw new Error(`Invalid cron expression!`);
    }
    validator_1.CronValidatorU2Q.validateQuartz(expanded);
 ...
 */
function matchesNonTimeFields(parsed, dp) {
    if (parsed.kind === 'unix') {
        const [, , dayOfMonth, month, dayOfWeek] = parsed.parts;
        return (fieldMatchesNumeric(month, dp.month, { min: 1, max: 12, aliases: MONTH_ALIASES }) &&
...
function matchesTimeFields(parsed, dp) {
    if (parsed.kind === 'unix') {
 ...
 */
function jumpToNextDayStart(cursor, timezone) {
    if (!timezone) {
...
function jumpToPreviousDayEnd(cursor, precision, timezone) {
...
function wallClockToAbsoluteMs(year, month, day, hour, minute, second, timezone) {
