...
function getNextRuns(expression, count, fromDate = new Date()) {
...
function matchesUnixExpression(parts, date) {
    const [minute, hour, dayOfMonth, month, dayOfWeek] = parts;
    return (fieldMatchesNumeric(minute, date.getMinutes(), { min: 0, max: 59 }) &&
...
function matchesUnixDayOfMonthAndWeek(dayOfMonth, dayOfWeek, date) {
...
function matchesQuartzExpression(parts, date) {
    const [second, minute, hour, dayOfMonth, month, dayOfWeek, year] = parts;
    return (fieldMatchesNumeric(second, date.getSeconds(), { min: 0, max: 59 }) &&
        ...
        fieldMatchesNumeric(year !== null && year !== void 0 ? year : '*', date.getFullYear(), { min: 1970, max: 2099 }));
}
function matchesQuartzDayOfMonth(field, date) {
    if (field === '*' || field === '?') {
        return true;
    }
...
function matchesQuartzDayOfWeek(field, date) {
    if (field === '*' || field === '?') {
        return true;
    }
