...
class CronConverterU2Q {
    ...
    static unixToQuartz(unixExpression, year = '*') {
        unixExpression = helper_1.ExpressionHelper.expandMacro(unixExpression);
    ...
    static parseUnixDowToken(token) {
    ...
    static parseQuartzDowToken(token) {
     ...
     */
    static renderSteppedDowValues(start, end, step, map) {
        const rendered = [];
        for (let value = start; value <= end; value += step) {
            const mapped = map(value).toString();
            if (!rendered.includes(mapped))
    ...
    static unixDowToQuartz(dow) {
        ...
        if (dow.includes('/')) {
            ...
            if (start.includes('-')) {
                ...
                endNum = (_a = this.parseUnixDowToken(rangeEnd)) !== null && _a !== void 0 ? _a : endNum;
            }
            else {
                startNum = this.parseUnixDowToken(start);
            }
            if (startNum === null || !Number.isInteger(stepNum) || stepNum <= 0)
                return dow;
        ...
        if (dow.includes('-')) {
            ...
            if (start === end)
                return this.unixDowToQuartz(start);
            ...
            if (end === '7') {
                ...
                if (startNum === 0)
                    return '*'; // 0-7 / SUN-7 covers every day
    ...
    static quartzDowToUnix(dow) {
        ...
        if (dow.includes('/')) {
            ...
            if (start.includes('-')) {
                ...
                endNum = (_a = this.parseQuartzDowToken(rangeEnd)) !== null && _a !== void 0 ? _a : endNum;
            }
            else {
        ...
        if (dow.includes('-')) {
            const [start, end] = dow.split('-');
            if (start === end)
                return this.quartzDowToUnix(start);
...
CronConverterU2Q.UNIX_DOW_ALIASES = ['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT'];
CronConverterU2Q.QUARTZ_DOW_ALIASES = ['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT'];
