...
exports.CronValidatorU2Q = void 0;
const helper_1 = require("./helper");
class FieldValidator {
    constructor(min, max, name, aliases = null, allowedSpecial = []) {
        this.min = min;
        this.max = max;
        this.name = name;
    ...
    validateSubPart(subPart, isQuartz) {
        if (subPart === "*")
            return;
        if (subPart === "?") {
            if (this.allowedSpecial.includes("?"))
                return;
        ...
        if (subPart.includes("/")) {
            ...
            // Validate the step start like any other value: for fields where 0 is
            // legal (second/minute/hour) it passes anyway, and for fields where it
            // is not (day-of-month, month, Quartz day-of-week) it must be rejected.
            if (start !== "*") {
                if (start.includes("-")) {
    ...
    validateSingleValue(value, isQuartz) {
        if (value === "") {
...
class CronValidatorU2Q {
    static validateUnix(expression) {
        if (typeof expression !== "string") {
            throw new Error("Cron expression must be a string");
        }
        expression = helper_1.ExpressionHelper.expandMacro(expression);
    ...
    static validateQuartz(expression) {
        if (typeof expression !== "string") {
            throw new Error("Cron expression must be a string");
        }
        const parts = expression.trim().split(/\s+/);
    ...
    static isValidQuartz(expression) {
        try {
...
exports.CronValidatorU2Q = CronValidatorU2Q;
