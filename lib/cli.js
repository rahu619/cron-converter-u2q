...
function main() {
    ...
    try {
        ...
        const nextRuns = (0, index_1.getNextRuns)(expression, options.count, options.fromDate);
        console.log(`Expression: ${expression}`);
        console.log(`Format: ${format}`);
        console.log(`Valid Unix: ${index_1.CronValidatorU2Q.isValidUnix(expression)}`);
        console.log(`Valid Quartz: ${index_1.CronValidatorU2Q.isValidQuartz(expression)}`);
        if (format === 'unix') {
            console.log(`Quartz: ${index_1.CronConverterU2Q.unixToQuartz(expression)}`);
            console.log(`Description: ${index_1.CronDescriberU2Q.describeUnix(expression)}`);
        }
        else {
            console.log(`Unix: ${index_1.CronConverterU2Q.quartzToUnix(expression)}`);
            console.log(`Description: ${index_1.CronDescriberU2Q.describeQuartz(expression)}`);
        }
...
function detectFormat(expression) {
    const isUnix = index_1.CronValidatorU2Q.isValidUnix(expression);
    const isQuartz = index_1.CronValidatorU2Q.isValidQuartz(expression);
    if (isUnix && !isQuartz) {
        return 'unix';
    }
    if (isQuartz && !isUnix) {
